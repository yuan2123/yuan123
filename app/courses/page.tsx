...
export default function CoursesPage() {
  return <>
    ...
    <section className="section section-dark learning-experience"><div className="container"><SectionHeading eyebrow="LEARNING EXPERIENCE" title="让知识进入体验，也进入家庭讨论"/><div className="level-grid">{[['01','主题授课','用清晰语言讲解财富、风险、现金流与家庭目标'],['02','财富流沙盘','在模拟决策中观察选择、结果与行为模式'],['03','绘画心理体验','从表达与互动中理解自己和家庭的真实需求'],['04','复盘沙龙','把课堂收获带回现实，在交流中持续校准行动']].map(([n,t,d])=><article key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p></article>)}</div></div></section>
  </>;
}
